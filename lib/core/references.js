...
export default class References {
  ...
  constructor(logger, eventBus) {
    ...
    eventBus.on('indexer:updated', (item) => {
      ...
      try {
        ...
        this._logger.log('references :: failed to process ' + item.uri, String(err));
      }
    });
  ...
  findReferences(ref) {
    ...
    const anchor = link?.anchor || this._findRef(this.anchorsByDocument, ref);
  ...
  _findRef(refs, ref) {
    ...
    return Array.from(potentialRefs).find(ref => this._inRange(position, ref.position));
  }
 ...
 * @return {boolean}
 */
function isDocumentRange(position) {
  return [
