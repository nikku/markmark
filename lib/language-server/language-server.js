...
export function createLanguageServer() {
  ...
  connection.onReferences((event) => {
    ...
    logger.log('connection.onReferences :: position', position);
  ...
  markmark.on('linter:lint', /** @param { LinterResults[] } reports */ (reports) => {
    ...
    for (const report of reports) {
      ...
      logger.log('markmark :: linter:lint :: add diagnostics', doc.uri, report);
    ...
    for (const oldUri in oldDiagnostics) {

      logger.log('markmark :: linter:lint :: clear diagnostics', oldUri);
    ...
    Promise.all(jobs).catch(err => {
      logger.log('markmark :: linter:lint :: failed to send diagnostics', err);
    });
  });
  ...
  connection.onDefinition((event) => {
    ...
    logger.log('connection.onDefinition :: position', position);
  ...
  connection.onCompletion(async (event, _, workDoneProgress) => {
    ...
    logger.log('connection.onCompletion :: get', uri, position);
    ...
    try {
      ...
      logger.log('ERRR', err.message, err.stack);
