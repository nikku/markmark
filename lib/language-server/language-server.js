 ...
 *
 * @typedef { import('../core/types.js').LinterResults } LinterResults
 *
 ...
 *
 * @param {VFileMessage & { position?: UnistPosition } } message
 * @returns {Diagnostic}
 */
...
export function createLanguageServer() {
  ...
  markmark.on('linter:lint', /** @param { LinterResults[] } reports */ (reports) => {
    ...
    for (const report of reports) {
      ...
      }).catch(err => logger.error('FAILED TO SEND DIAGNISTOCS', err)));
  ...
  connection.onCompletion(async (event, _completionItem, _workDoneProgress) => {
  ...
  connection.onInitialize((event, _cancelationToken, workDoneProgress, _resultProgress) => {
    ...
    logger.info('client support :: ', {
      fileWatching: hasFileWatchingSupport,
      workspaceFolders: hasWorkspaceFolderSupport
    });

    if (hasWorkspaceFolderSupport) {
