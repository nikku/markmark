...
export function createLanguageServer() {
  ...
  connection.onDocumentLinks((event) => {
    ...
    logger.info('connection.onDocumentLinks', event);

    const uri = event.textDocument.uri;

    const links = markmark.findDocumentLinks(uri);

    logger.info('connection.onDocumentLinks :: links', links);

    return links.map(
      link => DocumentLink.create(unistLocationToLspRange(link.position), link.targetUri)
    );
  });
